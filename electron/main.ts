...
// Global error traps to prevent application crashes from background socket/client errors
process.on('unhandledRejection', (reason) => {
  console.error('[MAIN] Intercepted Unhandled Rejection:', reason);
});

process.on('uncaughtException', (error) => {
  console.error('[MAIN] Intercepted Uncaught Exception:', error);
});
...
// GPU stability flags — ensure hardware and software fallback both work smoothly
...
function createWindow() {
  ...
  // Disable developer tools and block inspection hotkeys only in packaged distribution
