...
export class SanitizerService {
    ...
    async leaveGroups(ids: string[], silent: boolean = false) {
        try {
            ...
            const targetIds: any[] = ids.length > 0 ? ids : Array.from(this.client.channels.cache.filter((c: any) => c.type === 'GROUP_DM').keys());
            this.logCallback(`Départ${silent ? ' (discret)' : ''} de ${targetIds.length} groupes...`, 'info');
            let count = 0;

            await this.runInChunks(targetIds, 2, 700, async (id: any) => {
    ...
    async deleteFriends(ids: string[]) {
        try {
            ...
            await this.runInChunks(targetIds, 2, 800, async (id: any) => {
                if (!this.isSanitizing) return;
                try {
    ...
    async closeAllDMs() {
        try {
            this.isSanitizing = true;
            const dmChannels = Array.from(this.client.channels.cache.filter((c: any) => c.type === 'DM' || c.type === 'GROUP_DM').values());
            this.logCallback(`Fermeture de ${dmChannels.length} conversations privées...`, 'info');
            let count = 0;

            await this.runInChunks(dmChannels, 2, 700, async (channel: any) => {
                if (!this.isSanitizing) return;
                try {
                    await (channel as any).delete();
                    count++;
                    if (count % 20 === 0) this.logCallback(`[Progression] Fermeture des DM : ${count}...`, 'info');
                } catch (e: any) { this.logCallback(`Erreur DM ${(channel as any)?.id || 'inconnu'} : ${e.message}`, 'error'); }
            });
