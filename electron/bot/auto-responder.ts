    ...
    private setupListener() {
        ...
        this.boundListener = async (message: Message) => {
            ...
            for (const rule of this.config.rules) {
                const cooldownKey = `${rule.trigger}_${message.channel.id}`;
                const lastTrigger = this.cooldowns.get(cooldownKey) || 0;
                // Minimum 5s cooldown per rule per channel to prevent rate-limit loops
                if (now - lastTrigger < 5000) continue;
