...
export class ProfileRotator {
    ...
    private async run() {
        ...
        const delay = Math.max(3000, (this.config.interval * 1000) + jitter);
        ...
        if (cfg.enabledSections.activity && cfg.customRPCs && cfg.customRPCs.length > 0) {
            ...
            try {
                ...
                if (rpc.showTimestamp) {
                    presence.timestamps = { start: rpc.startTimestamp || Date.now() };
                }
                ...
                if (rpc.buttons && Array.isArray(rpc.buttons) && rpc.buttons.length > 0) {
                    const validButtons = rpc.buttons.slice(0, 2).filter(b => b.label && b.url);
                    if (validButtons.length > 0) {
                        activityObj.buttons = validButtons.map(b => b.label);
                        activityObj.metadata = { button_urls: validButtons.map(b => b.url) };
                    }
                }
                ...
                const activities: any[] = [activityObj];
