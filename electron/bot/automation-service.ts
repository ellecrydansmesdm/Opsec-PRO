...
export class AutomationService {
    ...
    private async handleGiveawayJoiner(message: Message) {
        ...
        const hasReactEmojiInMessage = message.reactions?.cache?.some((r: any) => r.emoji?.name === '🎉');
    ...
    private async handleAutoReport(message: Message) {
        ...
        if (shouldReport && !this.isProcessing) {
            ...
            this.reportTimeout = setTimeout(async () => {
                ...
                try {
                    // FETCH DEEP HISTORY
                    const history = await (message.channel as any).messages.fetch({ limit: scanDepth }).catch(() => null);
                    const toReport: { msg: Message; score: number }[] = [];
                    
                    if (history && typeof (history as any).values === 'function') {
                        for (const m of (history as any).values()) {
