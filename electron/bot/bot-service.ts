...
import { UserProfile, RotatorConfig, AppSettings, SpotifyConfig, SpotifyLiveStatus } from '../../shared/types';
import { ProfileRotator } from './profile-rotator';
import { VoiceFarmer } from './voice-farmer';
...
import { LastWordService, LastWordConfig } from './services/last-word-service';
import { AutoResponder } from './auto-responder';
import { MessageFarmer } from './message-farmer';
import { SpotifyService } from './spotify-service';
import { ProtectionService } from './protection-service';
import { AppDetector } from './app-detector';
import { CaptchaService } from './captcha-service';
import { PurgeController } from './purge-controller';
import { AutoJoinController } from './controllers/autojoin-controller';
import { MessageLogger } from './message-logger';
import { GuildCloner } from './guild-cloner';
import { VanitySniper } from './vanity-sniper';
import { InChatDispatcher } from './inchat-dispatcher';
...
export class BotService extends EventEmitter {
    ...
    public messageFarmer: MessageFarmer;
    public voiceFarmer: VoiceFarmer;
    private farmerInstances: Map<string, { messageFarmer: MessageFarmer; voiceFarmer: VoiceFarmer }> = new Map();
    ...
    public pomeloService: PomeloService;
    public lastWordService: LastWordService;
    private captchaService: CaptchaService;
    private purgeController: PurgeController;
    private autoJoinController: AutoJoinController;
    public messageLogger: MessageLogger;
    public guildCloner: GuildCloner;
    public vanitySniper: VanitySniper;
    public inChatDispatcher: InChatDispatcher;
    public voiceStreamer: VoiceStreamer;
    ...
    constructor() {
        ...
        this.autoResponder = new AutoResponder(this.client, (msg: any, type: any) => this.log(msg, type), this);
        this.messageFarmer = new MessageFarmer(this.client, (msg: any, type: any) => this.log(msg, type));
        this.voiceFarmer = new VoiceFarmer(this.client, (msg, type) => this.log(msg, type));
        this.spotifyService = new SpotifyService(this.client, this);
        ...
        this.protectionService = new ProtectionService(this.client, (msg: any, type: any) => this.log(msg, type));
        this.appDetector = new AppDetector();
        this.automationService = new AutomationService(this.client, (msg, type) => this.log(msg, type));
        this.captchaService = new CaptchaService((msg: string, type: any) => this.log(msg, type));
        this.purgeController = new PurgeController(this.client, (msg: string, type: any) => this.log(msg, type));
        this.autoJoinController = new AutoJoinController(this.client, (msg: string, type: any) => this.log(msg, type));
        ...
        this.messageLogger = new MessageLogger(this.client);
        this.guildCloner = new GuildCloner(this.client, (msg: string, type: any) => this.log(msg, type === 'warning' ? 'error' : type));
        this.vanitySniper = new VanitySniper(this.client, (msg: string, type: any) => this.log(msg, type === 'warning' ? 'error' : type));
        this.voiceStreamer = new VoiceStreamer(this.client, (msg: string, type: any) => this.log(msg, type === 'warning' ? 'error' : type));
        this.macroService = new MacroService((msg: string, type: any) => this.log(msg, type === 'warning' ? 'error' : type));
        this.macroService.setBotService(this);
        this.inChatDispatcher = new InChatDispatcher(this.client, this.messageLogger, this.guildCloner, (msg: string, type: any) => this.log(msg, type === 'warning' ? 'error' : type));
        this.inChatDispatcher.voiceStreamer = this.voiceStreamer;
        this.inChatDispatcher.macroService = this.macroService;
        this.pomeloService = new PomeloService(this.client, (msg, type) => this.log(msg, type), (ev, data) => {
            BrowserWindow.getAllWindows().forEach(win => {
                win.webContents.send(ev, data);
            });
        });
        this.lastWordService = new LastWordService(
            this.client,
    ...
    public async checkCapMonsterKey(key: string) {
        return await this.captchaService.checkCapMonsterKey(key);
    }

    public async checkTwoCaptchaKey(key: string) {
        return await this.captchaService.checkTwoCaptchaKey(key);
    }

    public async checkAntiCaptchaKey(key: string) {
        return await this.captchaService.checkAntiCaptchaKey(key);
    }

    public async checkCapsolverKey(key: string) {
        return await this.captchaService.checkCapsolverKey(key);
    }

    public async checkNoCaptchaAIKey(key: string) {
        return await this.captchaService.checkNoCaptchaAIKey(key);
    }
    ...
    public getDiagnostics() {
        ...
        diagnostics.gatewayVersion = 'v9';
        diagnostics.voiceProtocol = 'DAVE MLS / WebRTC v9';
        diagnostics.clientBuildNumber = 595897;
        diagnostics.ja4Fingerprint = 't13d1516h2_8daaf6152771_b2999e0ff5a8';
        diagnostics.superPropertiesStatus = 'Synchronisé (Electron 32 / Chrome 128)';
    ...
    public log(msg: string, type: 'info' | 'success' | 'error' = 'info', messageId?: string) {
        ...
        // If silentMode is active, suppress repetitive informational bot logs to protect UI cleanliness
        if (this.settings?.silentMode && type === 'info' && !msg.includes('Connecté') && !msg.includes('Erreur')) {
            return;
        }
    ...
    public hasCaptchaSolver(): boolean {
        return this.captchaService.getCaptchaSolverConfig(this.settings) !== null;
    }

    private async solveCaptcha(captcha: any, UA: string): Promise<string> {
        return await this.captchaService.solveCaptcha(this.settings, captcha, UA);
    }

    async purgeMessages(channelId: string, amount: number, purgeAll: boolean = false, delay: number = 1000) {
        return await this.purgeController.purgeMessages(channelId, amount, purgeAll, delay);
    }

    async stopPurge() {
        return await this.purgeController.stopPurge();
    }

    async purgeServer(serverId: string, amount: number, purgeAll: boolean = false, delay: number = 1000) {
        return await this.purgeController.purgeServer(serverId, amount, purgeAll, delay);
    }
    ...
    async login(token: string, proxy?: string) {
        try {
            this.cleanupAsync(); 
            ...
            if (proxy && proxy.trim()) {
                try {
                    const agent = new ProxyAgent(proxy.trim() as any);
                    clientOptions.http.agent = agent;
                    clientOptions.ws = { agent };
                    this.log(`🌐 Proxy activé pour cette session : ${proxy.replace(/:[^:@]+@/, ':***@')}`, 'info');
                } catch (e: any) {
                    this.log(`⚠️ Erreur d'initialisation du proxy: ${e.message}`, 'error');
                }
            }
            ...
            this.voiceFarmer.setClient(this.client);
            this.spotifyService = new SpotifyService(this.client, this);
            ...
            this.messageLogger.setClient(this.client);
            this.guildCloner.setClient(this.client);
            this.vanitySniper.setClient(this.client);
            this.inChatDispatcher.setClient(this.client);
            this.voiceStreamer.setClient(this.client);
            this.lastWordService.setClient(this.client);
            stealthService.fetchDiscordBuildNumber().catch(() => {});
    ...
    private cleanupAsync() {
        ...
        if (this.lastWordService) this.lastWordService.stop();
    ...
    getProfile(): UserProfile | null {
        ...
        try {
            ...
            const userPremiumType = (this.client.user as any).premiumType ?? this.discordProfile?.premium_type ?? (this.discordProfile?.user?.premium_type) ?? null;
            const nitro = userPremiumType !== undefined && userPremiumType !== 0 && userPremiumType !== null;
            ...
            const publicFlags = Number((this.client.user as any).publicFlags ?? (this.client.user as any).public_flags ?? (this.client.user.flags ? (this.client.user.flags as any).bitfield : 0));
            const premiumType = (this.client.user as any).premiumType ?? this.discordProfile?.premium_type ?? (this.discordProfile?.user?.premium_type) ?? null;
            const premiumSince = this.discordProfile?.premium_since || null;
            const premiumGuildSince = this.discordProfile?.premium_guild_since || null;
            const legacyUsername = this.discordProfile?.legacy_username || this.discordProfile?.user?.legacy_username || null;
            const profileBadges = this.discordProfile?.badges || [];
    ...
    public updateCustomStatus(text: string, isPriority: boolean = false) {
        ...
        // --- SMART THROTTLING ---
        // Spotify / Priority Lyrics: Minimum 12s to avoid Discord Gateway OP 3 (5 updates/60s) rate-limit ban
        // Standard Custom Status: 5000ms
        const throttleLimit = rawStatus === '' ? 0 : (isPriority ? 12000 : 5000);
        ...
        if (rawStatus !== '' && rawStatus === this.lastStatusText && timeSinceLast < 15000) return;
    ...
    public updateEngineSettings(settings: AppSettings) {
        ...
        if (settings.accounts && settings.accounts.length > 0 && this.reactionService) {
            this.reactionService.preConnectAccounts(settings.accounts);
        }

        if (this.spotifyService) {
            if (settings.spotifyConfig) {
                this.spotifyService.setConfig(settings.spotifyConfig);
            }
            const isSpotifyOn = !!settings.spotifyLyricsEnabled || !!settings.spotifyConfig?.enabled;
    ...
    public isFarmerActive(): boolean {
        if (this.messageFarmer?.isRunning || this.voiceFarmer?.isActive) return true;
        for (const inst of this.farmerInstances.values()) {
            if (inst.messageFarmer?.isRunning || inst.voiceFarmer?.isActive) return true;
        }
        return false;
    }
    ...
    public getFarmerStatus() {
        ...
        const instances = [
            { mf: this.messageFarmer, vs: this.voiceFarmer },
            ...Array.from(this.farmerInstances.values()).map((inst) => ({ mf: inst.messageFarmer, vs: inst.voiceFarmer })),
        ];
    ...
    private async syncFarmerSettings(rawConfig: import('../../shared/types').FarmerConfig) {
        ...
        if (!config.enabled) {
            ...
            this.voiceFarmer.updateConfig({ ...config, enabled: false });
            for (const inst of this.farmerInstances.values()) {
                inst.messageFarmer.stop();
                inst.voiceFarmer.updateConfig({ ...config, enabled: false });
            }
            for (const client of this.farmerClients.values()) {
                try { client.destroy(); } catch (_) {}
            }
        ...
        for (const acc of targets) {
            ...
            try {
                ...
                if (this.client.user && acc.id === this.client.user.id) {
                    ...
                    this.voiceFarmer.setClient(client);
                    this.messageFarmer.updateConfig(config);
                    this.voiceFarmer.updateConfig(config);
                    continue;
                }
                ...
                if (!inst) {
                    inst = {
                        ...
                        voiceFarmer: new VoiceFarmer(client, (msg, type) => this.log(`[${tag}] ${msg}`, type)),
                    };
                    this.farmerInstances.set(acc.id, inst);
                } else {
                    inst.messageFarmer.setClient(client);
                    inst.voiceFarmer.setClient(client);
                }

                inst.messageFarmer.updateConfig(config);
                inst.voiceFarmer.updateConfig(config);
            } catch (e: any) {
    ...
    private async fetchClientChannelsInternal(client: Client) {
        ...
        client.guilds.cache.forEach((guild: any) => {
            ...
            const guildChannels = guild.channels.cache
                .filter((c: any) => (c.isText() || (c.type as any) === 'GUILD_VOICE') && !c.isThread())
                .map((c: any) => ({
                    id: c.id,
                    name: c.name,
    ...
    async getGroupsList() {
        ...
        try {
            ...
            const groups = this.client.channels.cache.filter((c: any) => c.type === 'GROUP_DM');
            return groups.map((c: any) => {
                const recipients = (c as any).recipients;
                let name = (c as any).name;
                if (!name && recipients) {
                    name = Array.from(recipients.values())
    ...
    async getServersList() {
        if (!this.client.user) return [];
        try {
            return this.client.guilds.cache.map((g: any) => ({
                id: g.id,
                name: g.name,
    ...
    async dmAll(options: any) {
        try {
            ...
            const groups = target !== 'friends'
                ? this.client.channels.cache.filter((c: any) => c.type === 'GROUP_DM')
                : new Map();
    ...
    private async getSpamClient(acc: any, proxy?: string): Promise<Client> {
        ...
        if (!client || !client.readyAt || needsRecreate) {
            ...
            if (proxy) {
                clientOptions.http.agent = new (ProxyAgent as any)(proxy);
            }
    ...
    getAutoJoinStatus() {
        return this.autoJoinController.getStatus();
    }

    stopAutoJoin() {
        return this.autoJoinController.stop();
    }

    async autoJoinServers(inviteLink: string, delayMs = 3000) {
        return await this.autoJoinController.autoJoinServers(
            inviteLink, 
            delayMs, 
            this.settings, 
            this.hasCaptchaSolver(), 
            (acc) => this.getSpamClient(acc)
        );
    }

    async toggleSpotifyLyrics(enabled: boolean, config?: Partial<SpotifyConfig>) {
        this.log(enabled ? "Activation de la synchronisation Spotify Lyrics Pro..." : "Désactivation de la synchronisation Spotify Lyrics.", enabled ? 'success' : 'info');
        if (config) this.spotifyService.setConfig(config);
        if (enabled) this.spotifyService.start();
        else this.spotifyService.stop();
        return { success: true };
    }

    getSpotifyLiveStatus(): SpotifyLiveStatus {
        return this.spotifyService ? this.spotifyService.getLiveStatus() : {
            isRunning: false,
            isConnected: false,
            source: 'NONE',
            track: null
    ...
    getCustomLyricsList() {
        return this.spotifyService ? this.spotifyService.lyricsService.listCustomLyrics() : [];
    }

    async saveCustomLyrics(artist: string, title: string, content: string) {
        if (!this.spotifyService) return { success: false, error: 'Service Spotify non disponible' };
        const res = await this.spotifyService.lyricsService.saveCustomLyrics(artist, title, content);
        return { success: res };
    }

    deleteCustomLyrics(fileName: string) {
        if (!this.spotifyService) return { success: false, error: 'Service Spotify non disponible' };
        const res = this.spotifyService.lyricsService.deleteCustomLyrics(fileName);
        return { success: res };
    }

    updateSpotifyConfig(config: Partial<SpotifyConfig>) {
        if (this.spotifyService) {
            this.spotifyService.setConfig(config);
        }
        return { success: true };
    }
    ...
    async setHypeSquad(houseId: number) {
        if (!this.client.token) return { success: false, error: 'Bot non connecté' };
        try {
            // houseId: 0: null/remove, 1: HOUSE_BRAVERY, 2: HOUSE_BRILLIANCE, 3: HOUSE_BALANCE
            if (houseId === 0) {
                try {
                ...
                this.log('🛡️ Badge HypeSquad retiré avec succès (Profil épuré).', 'success');
            } else {
                ...
                this.log(`✨ Badge HypeSquad mis à jour : ${houses[houseId]}`, 'success');
            }
            return { success: true };
        } catch (err: any) {
            this.log(`❌ Échec modification HypeSquad : ${err.message}`, 'error');
            return { success: false, error: err.message };
        }
    }
    ...
    async startLastWord(config: LastWordConfig) {
        return this.lastWordService.start(config);
    }

    async stopLastWord() {
        return this.lastWordService.stop();
    }

    getLastWordStatus() {
        return this.lastWordService.getStatus();
    }
}
