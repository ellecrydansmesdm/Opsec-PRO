...
import { getSettings, saveSettings, resetSettingsToDefault } from '../utils/settings';
...
export function setupIpcHandlers(win: BrowserWindow | null, botService: BotService | null, accountManager: AccountManager | null) {
  ...
  let sessionLicenseValidated = false;
  let sessionLicenseKey = '';

  const checkLicenseGuard = (): boolean => {
    if (sessionLicenseValidated && sessionLicenseKey) {
      return true;
    }
    const settings = getSettings();
    if (settings.licenseKey && settings.licenseValidated) {
      sessionLicenseValidated = true;
      sessionLicenseKey = settings.licenseKey;
      return true;
    }
    return false;
  };
  ...
  ipcMain.handle('check-auth', async (_, data?: { licenseKey?: string }) => {
    try {
      ...
      if (licenseKey) {
        ...
        // Unlock in-memory session state immediately
        sessionLicenseValidated = true;
        sessionLicenseKey = licenseKey;
        ...
        return { success: true, data: { authenticated: false, requireLicense: false } }; // Let login flow continue
      }
      ...
      if (!revalidation.success) {
        const isDefinitiveRejection = revalidation.message?.includes('invalide') || 
                                     revalidation.message?.includes('expir') || 
                                     revalidation.message?.includes('désactivée') || 
                                     revalidation.message?.includes('bannie') || 
                                     revalidation.message?.includes('autre ordinateur');
        if (isDefinitiveRejection) {
          console.warn('[AUTH] Silent re-validation explicitly rejected key. Reason:', revalidation.message);
          sessionLicenseValidated = false;
          sessionLicenseKey = '';
          saveSettings({ ...settings, licenseValidated: false });
          return { success: true, data: { authenticated: false, requireLicense: true, savedKey: settings.licenseKey } };
        } else {
          console.warn('[AUTH] Temporary network or server issue during silent check. Preserving cached validation. Reason:', revalidation.message);
          sessionLicenseValidated = true;
          sessionLicenseKey = settings.licenseKey;
        }
      } else {
  ...
  ipcMain.handle('login-attempt', async (_, { token, rememberMe, proxy }) => {
    if (!checkLicenseGuard()) {
      return { success: false, error: 'Accès refusé : Licence Opsec PRO requise.' };
    }
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    try {
      const result = await botService.login(token, proxy);
  ...
  ipcMain.handle('batch-validate-tokens', async (_, { tokens, rememberMe }: { tokens: string[]; rememberMe: boolean }) => {
    if (!checkLicenseGuard()) {
      return { success: false, error: 'Accès refusé : Licence Opsec PRO requise.' };
    }
    if (!accountManager) return { success: false, error: 'AccountManager non initialisé' };

    const newAccountsToSave: any[] = [];
    const results: { token: string; user?: any; error?: string }[] = [];

    // Parallel fetch validation with concurrency limit of 10
    const concurrencyLimit = 10;
    for (let i = 0; i < tokens.length; i += concurrencyLimit) {
      const chunk = tokens.slice(i, i + concurrencyLimit);
      await Promise.all(
          ...
          try {
            ...
            if (res.ok) {
              ...
              const userObj = {
                ...
                avatarURL: userData.avatar ? `https://cdn.discordapp.com/avatars/${userData.id}/${userData.avatar}.png` : ''
              };

              results.push({ token, user: userObj });
              ...
              if (rememberMe) {
                newAccountsToSave.push({
                  ...
                  rotator: {
                    ...
                    currentActivityIndex: 0,
                    currentClanTagIndex: 0,
                    enabledSections: { status: true, bio: true, username: false, activity: true, clanTag: false },
                    hypesquadHouse: 0,
                    stats: { messagesToday: 0, totalMessages: 0 },
                    totalRotations: 0
                  }
                });
              }
            } else {
              results.push({ token, error: `Invalid Token (HTTP ${res.status})` });
            }
          } catch (err: any) {
            results.push({ token, error: err.message || 'Network Error' });
          }
        })
      );
    }
    ...
    if (newAccountsToSave.length > 0) {
      accountManager.addAccountsBatch(newAccountsToSave);
      const current = getSettings();
      saveSettings({
        ...current,
        autoLogin: rememberMe,
      ...
      if (botService) botService.updateEngineSettings(getSettings());
    }
  ...
  if (account) {
      ...
      const res = await botService.login(account.token, account.proxy);
      if (res.success && res.user) {
          // Update stored user details in accountManager with fresh Discord profile
          accountManager.updateAccount(res.user.id, {
  ...
  ipcMain.handle('save-settings', (_, updated) => {
    try {
      ...
      if (!sanitizedUpdated.licenseKey && current.licenseKey) {
        sanitizedUpdated.licenseKey = current.licenseKey;
        sanitizedUpdated.licenseValidated = current.licenseValidated;
      }
      const newSettings = { ...current, ...sanitizedUpdated };
  ...
  ipcMain.handle('reset-settings', () => {
    try {
      ...
      if (botService) {
        botService.updateEngineSettings(reset);
      }
      notifySettingsUpdate(mainWindow);
  ...
  ipcMain.handle('toggle-spotify-lyrics', async (_, { enabled, cookie, config }) => {
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    ...
    if (config) {
      settings.spotifyConfig = { ...(settings.spotifyConfig || {}), ...config, enabled };
    }
    saveSettings(settings);

    return await botService.toggleSpotifyLyrics(enabled, config);
  });

  ipcMain.handle('get-spotify-live-status', async () => {
    if (!botService) return { isRunning: false, isConnected: false, source: 'NONE', track: null };
    return botService.getSpotifyLiveStatus();
  });

  ipcMain.handle('get-custom-lyrics-list', async () => {
    if (!botService) return [];
    return botService.getCustomLyricsList();
  });

  ipcMain.handle('save-custom-lyrics', async (_, { artist, title, content }) => {
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    return await botService.saveCustomLyrics(artist, title, content);
  });

  ipcMain.handle('delete-custom-lyrics', async (_, fileName) => {
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    return botService.deleteCustomLyrics(fileName);
  });

  ipcMain.handle('save-spotify-config', async (_, config) => {
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    const settings = getSettings();
    settings.spotifyConfig = { ...(settings.spotifyConfig || {}), ...config };
    saveSettings(settings);
    return botService.updateSpotifyConfig(config);
  });
  ...
  ipcMain.handle('pomelo:get-status', async () => {
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    return { success: true, data: { running: botService.pomeloService.isPomeloBatching } };
  });
  ...
  // --- Last Word (Cardio Duel) Handlers ---
  ipcMain.handle('lastword:start', async (_, config) => {
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    return await botService.startLastWord(config);
  });

  ipcMain.handle('lastword:stop', async () => {
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    return await botService.stopLastWord();
  });

  ipcMain.handle('lastword:get-status', async () => {
    if (!botService) return { success: false, error: 'Bot service non initialisé' };
    return { success: true, data: botService.getLastWordStatus() };
  });
  ...
  ipcMain.handle('test-proxies', async (_, proxyList: string[]) => {
    ...
    try {
      ...
      const results = await Promise.all(
        proxyList.slice(0, 50).map(async (rawProxy) => {
          const trimmed = rawProxy.trim();
          if (!trimmed) return null;
          let formatted = trimmed;
          if (!formatted.startsWith('http://') && !formatted.startsWith('https://') && !formatted.startsWith('socks4://') && !formatted.startsWith('socks5://')) {
            formatted = 'socks5://' + formatted;
          }

          const startTime = Date.now();
          try {
            const agent = new (ProxyAgent as any)(formatted);
            ...
            return { proxy: trimmed, status: response.ok ? 'online' : 'error', latencyMs };
          } catch (err: any) {
            return { proxy: trimmed, status: 'dead', latencyMs: null, error: err.message || 'Timeout' };
          }
        })
      );
  ...
  ipcMain.handle('check-for-updates', async () => {
    return await UpdateChecker.checkForUpdates(app.getVersion());
  });

  ipcMain.handle('install-update-and-quit', async (_, downloadUrl?: string) => {
    try {
      if (downloadUrl) {
  ...
  // --- In-Chat Command Dispatcher Handlers ---
  ipcMain.handle('inchat:toggle', (_, enabled: boolean) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    botService.inChatDispatcher.enabled = enabled;
    return { success: true, enabled: botService.inChatDispatcher.enabled };
  });

  ipcMain.handle('inchat:set-prefix', (_, prefix: string) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    botService.inChatDispatcher.prefix = prefix || '.';
    return { success: true, prefix: botService.inChatDispatcher.prefix };
  });

  ipcMain.handle('inchat:get-status', () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return { 
      success: true, 
      data: { 
        enabled: botService.inChatDispatcher.enabled, 
        prefix: botService.inChatDispatcher.prefix 
      } 
    };
  });

  // --- Guild Cloner & Backup Handlers ---
  ipcMain.handle('backup:create', async (_, guildId: string) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return await botService.guildCloner.createBackup(guildId);
  });

  ipcMain.handle('backup:list', () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return { success: true, data: botService.guildCloner.listBackups() };
  });

  ipcMain.handle('backup:load', async (_, fileName: string) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    const data = await botService.guildCloner.loadBackupFromFile(fileName);
    return { success: !!data, data };
  });

  ipcMain.handle('backup:clone', async (_, { backup, targetGuildId, clearExisting, delayMs }) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return await botService.guildCloner.cloneGuild(backup, targetGuildId, { clearExisting, delayMs });
  });

  // --- Vanity Sniper Handlers ---
  ipcMain.handle('vanity:start', async (_, data: any) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    const targetGuildId = data.targetGuildId;
    const vanityCodes = data.vanityCodes || (data.vanityCode ? [data.vanityCode] : []);
    const delayMs = data.delayMs || 500;
    const botTokens = data.botTokens || [];
    const webhookUrl = data.webhookUrl;
    const gatewayListen = data.gatewayListen ?? true;

    return await botService.vanitySniper.start({
      targetGuildId,
      vanityCodes,
      delayMs,
      botTokens,
  ...
  ipcMain.handle('vanity:stop', () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return botService.vanitySniper.stop();
  });

  ipcMain.handle('vanity:status', () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return { success: true, data: botService.vanitySniper.getStatus() };
  });

  ipcMain.handle('vanity:check-guild', (_, guildId: string) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    const res = botService.vanitySniper.checkGuildEligibility(guildId);
    return { success: true, data: res };
  });

  // --- Message Snipe Handlers ---
  ipcMain.handle('snipe:get', (_, { channelId, index }: { channelId: string; index?: number }) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return { success: true, data: botService.messageLogger.getSnipe(channelId, index ?? 0) };
  });

  ipcMain.handle('snipe:get-edit', (_, { channelId, index }: { channelId: string; index?: number }) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return { success: true, data: botService.messageLogger.getEditSnipe(channelId, index ?? 0) };
  });

  // --- Stealth & Anti-Detection Handlers ---
  ipcMain.handle('stealth:get-build-number', async () => {
    const { stealthService } = require('../services/stealth-service');
    const build = await stealthService.fetchDiscordBuildNumber();
    return { success: true, buildNumber: build };
  });

  ipcMain.handle('stealth:get-headers', () => {
    const { stealthService } = require('../services/stealth-service');
    const headers = stealthService.getStealthHeaders();
    const superProps = stealthService.getSuperProperties();
    return { success: true, headers, superProps };
  });

  // --- Voice Channel Audio Streamer Handlers ---
  ipcMain.handle('voice:join', async (_, channelId: string) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return await botService.voiceStreamer.join(channelId);
  });

  ipcMain.handle('voice:leave', async () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return await botService.voiceStreamer.leave();
  });

  ipcMain.handle('voice:play', async (_, source: string) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return await botService.voiceStreamer.play(source);
  });

  ipcMain.handle('voice:stop', () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return botService.voiceStreamer.stop();
  });

  ipcMain.handle('voice:status', () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return { success: true, data: botService.voiceStreamer.getStatus() };
  });

  ipcMain.handle('voice:set-volume', (_, volume: number) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return botService.voiceStreamer.setVolume(volume);
  });

  // --- Macro Engine Handlers ---
  ipcMain.handle('macro:list', () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return { success: true, data: botService.macroService.listMacros() };
  });

  ipcMain.handle('macro:save', (_, macro: any) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return botService.macroService.saveUserMacro(macro);
  });

  ipcMain.handle('macro:delete', (_, macroId: string) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return botService.macroService.deleteUserMacro(macroId);
  });

  ipcMain.handle('macro:execute', async (_, macroId: string) => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return await botService.macroService.executeMacro(macroId);
  });

  ipcMain.handle('macro:cancel', () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    botService.macroService.cancelExecution();
    return { success: true };
  });

  ipcMain.handle('macro:restore-snapshot', async () => {
    if (!botService) return { success: false, error: 'Bot non initialisé' };
    return await botService.macroService.restoreSnapshot();
  });

  ipcMain.handle('audit:get-history', (_, level?: any) => {
    return { success: true, data: appBus.getHistory(level) };
  });
}
