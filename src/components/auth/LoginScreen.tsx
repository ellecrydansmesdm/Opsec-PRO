...
export const LoginScreen = ({ onLogin, isModal, onClose }: LoginScreenProps) => {
  ...
  const handleBulkSubmit = async () => {
    ...
    setLoading(true);
    setError(null);
    setProcessingResults([{ status: `VALUATION DE ${tokens.length} TOKENS EN PARALLÈLE...`, color: 'var(--accent)' }]);
    
    try {
      const res = await window.electronAPI.batchValidateTokens({ tokens, rememberMe }) as any;
      if (res.success && Array.isArray(res.results)) {
        const mapped = res.results.map((r: any, idx: number) => {
          if (r.user) {
            return { tag: r.user.tag, status: `ACCOUNT ${idx+1}/${tokens.length} : SUCCESS (${r.user.tag})`, color: '#10b981' };
          }
          return { status: `ACCOUNT ${idx+1}/${tokens.length} : ERROR (${r.error || 'Token Invalide'})`, color: '#ff4444' };
        });
        setProcessingResults(mapped);
      } else {
        setError(res.error || 'Échec de la validation par lot');
      }
    } catch (err: any) {
