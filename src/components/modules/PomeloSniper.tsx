...
export const PomeloSniper = ({ showToast }: PomeloSniperProps) => {
    ...
    useEffect(() => {
        ...
        if (window.electronAPI.getPomeloStatus) {
            window.electronAPI.getPomeloStatus().then(res => {
        ...
        const unsubscribe = window.electronAPI.onPomeloUpdate((data: any) => {
            if (data.status === 'batch_ended' || data.status === 'batch_stopped') {
    ...
    const tokenCount = React.useMemo(() => {
        return botToken.split(/[\n,;]+/).map(t => t.trim()).filter(t => t.length > 10).length;
    }, [botToken]);
    ...
    return (
        <div className="glass-card animate-fade-in" style={{ padding: '30px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', opacity: 0.8 }}>
                            ...
                            <p className="caption" style={{ fontSize: '10px', fontWeight: 'bold', color: 'var(--accent)', textTransform: 'none' }}>
                                {settings.language === 'fr' 
                                    ? "Architecture 2-Tiers : 100% sécurisée — Vos tokens de Bot scannent, votre compte ne fait qu'une seule requête à la seconde du claim."
                                    : "2-Tier Architecture: 100% secure — Bot tokens scan 24/7, your personal account only fires 1 instant request when available."
                                }
                            </p>
                        </div>
                    </div>
                ...
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                    <button
                        ...
                        style={{ padding: '8px 12px', fontSize: '10px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '6px' }}
                    >
                        <Sparkles size={12} color="var(--accent)" />
                        {settings.language === 'fr' ? 'Créer un Bot Token' : 'Create Bot Token'}
                    </button>
            ...
            {/* OPSEC Zero-Risk Educational Tips Banner */}
            <div style={{ 
                marginBottom: '25px', 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }} onClick={() => setShowGuide(!showGuide)}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <span style={{ fontSize: '12px', fontWeight: '900', color: 'white', letterSpacing: '0.5px' }}>
                            {settings.language === 'fr' ? '🛡️ GUIDE OPSEC : COMMENT SNIPER SANS AUCUN RISQUE SUR VOTRE COMPTE' : '🛡️ OPSEC GUIDE: HOW TO SNIPE WITH ZERO RISK TO YOUR ACCOUNT'}
                        </span>
                    </div>
                    <span style={{ fontSize: '10px', color: 'var(--accent)', fontWeight: 'bold' }}>
                        {showGuide ? (settings.language === 'fr' ? 'Masquer' : 'Hide') : (settings.language === 'fr' ? 'Afficher les astuces' : 'Show Tips')}
                    </span>
                </div>

                {showGuide && (
                    <div style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px', fontSize: '11px', lineHeight: '1.5', color: 'var(--text-muted)' }}>
                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            <strong style={{ color: '#00d2ff', display: 'block', marginBottom: '3px' }}>1. Scanner Découplé (Bot Tokens)</strong>
                            {settings.language === 'fr' 
                                ? "Ne scannez jamais avec votre token perso. Créez des Bots gratuits sur le Discord Developer Portal et collez leurs tokens ici pour absorber les requêtes et rate-limits."
                                : "Never scan with your personal token. Create free Bots on the Developer Portal and paste their tokens here to absorb all rate limits."
                            }
                        </div>
                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            <strong style={{ color: '#62ff41', display: 'block', marginBottom: '3px' }}>2. Auto-Claim en Veille Passive</strong>
                            {settings.language === 'fr' 
                                ? "Votre compte perso reste en veille. Quand un bot trouve un pseudo disponible, votre compte envoie exactement 1 seule requête locale pour s'attribuer le pseudo."
                                : "Your personal account stays dormant. When a bot finds an available username, your account sends exactly 1 instant local request."
                            }
                        </div>
                        <div style={{ background: 'rgba(0,0,0,0.3)', padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            <strong style={{ color: '#c084fc', display: 'block', marginBottom: '3px' }}>3. Multi-Bot Tokens (Round-Robin)</strong>
                            {settings.language === 'fr' 
                                ? "Collez plusieurs Bot Tokens (séparés par des virgules ou retours à la ligne). Le scanner tourne en boucle sur tous les bots pour multiplier la cadence par 10."
                                : "Paste multiple Bot Tokens separated by commas. The scanner rotates across all bots to multiply speed by 10."
                            }
                        </div>
                    </div>
                )}
            ...
            <div style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <label className="caption" style={{ fontSize: '10px', color: 'var(--accent)', fontWeight: 'bold' }}>
                        {settings.language === 'fr' ? 'POOL DE BOT TOKENS (SÉPARÉS PAR VIRGULES OU RETOURS À LA LIGNE)' : 'BOT TOKEN POOL (SEPARATED BY COMMAS OR NEWLINES)'}
                    </label>
                    <span style={{ 
                        fontSize: '10px', 
                        fontWeight: '900', 
                        padding: '2px 8px', 
                        borderRadius: '6px', 
                        background: tokenCount > 0 ? 'rgba(0, 210, 255, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                        color: tokenCount > 0 ? 'var(--accent)' : 'var(--danger)'
                    }}>
                        {tokenCount > 0 ? `${tokenCount} Bot Token(s) Actif(s)` : (settings.language === 'fr' ? '0 Token (Requis)' : '0 Tokens (Required)')}
                    </span>
                </div>
                <textarea 
                    value={botToken}
                    onChange={e => setBotToken(e.target.value)}
                    placeholder={settings.language === 'fr' ? "MTQ... (Collez un ou plusieurs Bot Tokens de Discord Developer Portal)" : "MTQ... (Paste one or more Bot Tokens from Discord Developer Portal)"}
                    style={{ 
                        width: '100%', 
                        height: '65px', 
                    ...
                    <div style={{ padding: '20px', background: 'rgba(0,0,0,0.25)', borderRadius: '18px', border: '1px solid var(--border)', flex: 1, display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                            ...
                            >
                                ...
                                <option value="2l">{settings.language === 'fr' ? 'Générateur : 2 Lettres (Rare / Ultra OG)' : 'Generator: 2 Letters (Rare / Ultra OG)'}</option>
                                <option value="2c">{settings.language === 'fr' ? 'Générateur : 2 Caractères (Alphanum)' : 'Generator: 2 Characters (Alphanum)'}</option>
                                <option value="3l">{settings.language === 'fr' ? 'Générateur : 3 Lettres (OG)' : 'Generator: 3 Letters (OG)'}</option>
                                <option value="3c">{settings.language === 'fr' ? 'Générateur : 3 Caractères (Alphanum)' : 'Generator: 3 Characters (Alphanum)'}</option>
                                <option value="4l">{settings.language === 'fr' ? 'Générateur : 4 Lettres' : 'Generator: 4 Letters'}</option>
                                <option value="4c">{settings.language === 'fr' ? 'Générateur : 4 Caractères' : 'Generator: 4 Characters'}</option>
                                <option value="5l">{settings.language === 'fr' ? 'Générateur : 5 Lettres' : 'Generator: 5 Letters'}</option>
                            </select>
                        </div>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                {statusList.length > 0 && (
                                    <>
                                        <button 
                                            onClick={() => {
                                                const available = statusList.filter(s => s.status === 'available').map(s => s.username).join('\n');
                                                if (!available) {
                                                    showToast(settings.language === 'fr' ? 'Aucun pseudo disponible à exporter' : 'No available usernames to export', 'danger');
                                                    return;
                                                }
                                                ...
                                                a.href = url;
                                                a.download = `pomelo_available_${Date.now()}.txt`;
                                                a.click();
                                                URL.revokeObjectURL(url);
                                                audioService.play('action_btn_primary');
                                                showToast(settings.language === 'fr' ? 'Export TXT téléchargé !' : 'TXT Export downloaded!', 'success');
                                            }}
                                            className="btn-secondary"
                                            style={{ padding: '4px 8px', fontSize: '9px', fontWeight: 'bold' }}
                                        >
                                            {settings.language === 'fr' ? 'Export TXT' : 'Export TXT'}
                                        </button>
                                        <button 
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                    <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: isBatchRunning ? 'var(--accent)' : 'var(--text-dim)', animation: isBatchRunning ? 'pulse 2s infinite' : 'none' }}></div>
                                    <span style={{ fontSize: '9px', fontWeight: 'bold', color: isBatchRunning ? 'var(--accent)' : 'var(--text-dim)' }}>
                                        {isBatchRunning ? 'SCANNING' : 'IDLE'}
                                    </span>
                                </div>
                            </div>
                        </div>
