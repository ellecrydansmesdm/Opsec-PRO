...
import { DiscordBadgesRow, DiscordBadgeProgressionCard } from "@/components/DiscordBadge";
...
export const Overview = ({ onSwitch, onAdd }: OverviewProps) => {
  ...
  const nitroDate = settings.nitroStartDate || user.premiumSince;
  const boostDate = settings.boostStartDate || user.premiumGuildSince;
  const hasProgression = Boolean(nitroDate || boostDate);
    ...
    <div className="animate-fade-in" style={{ padding: '20px', height: 'calc(100vh - 32px)', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div className="dashboard-grid" style={{ flex: 1, display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '16px', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                    <DiscordBadgesRow 
                      userProfile={{
                        ...
                        premiumSince: nitroDate,
                        premiumGuildSince: boostDate,
                        legacyUsername: user.legacyUsername,
                        profileBadges: user.profileBadges,
                        rawBadgesList: user.badges
                      }}
          ...
          {/* Badge Progression Tracker (Only rendered if Nitro or Boost dates are active) */}
          {hasProgression && (
            <div style={{ display: 'grid', gridTemplateColumns: nitroDate && boostDate ? '1fr 1fr' : '1fr', gap: '12px', flexShrink: 0 }}>
              {nitroDate && (
                <DiscordBadgeProgressionCard type="nitro" startDate={nitroDate} language={settings.language} />
              )}
              {boostDate && (
                <DiscordBadgeProgressionCard type="boost" startDate={boostDate} language={settings.language} />
              )}
            </div>
          )}
