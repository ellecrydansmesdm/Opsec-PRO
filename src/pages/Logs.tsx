...
export const Logs = () => {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'success' | 'error' | 'info' | 'security'>('all');
  ...
  const filteredLogs = logs.filter(l => {
    ...
    if (typeFilter === 'all') return matchesText;
    if (typeFilter === 'security') return matchesText && (l.msg.includes('[SECURITY') || l.type === 'error');
    return matchesText && l.type === typeFilter;
  });
    ...
    <div className="animate-fade-in" style={{ height: 'calc(100% - 10px)', display: 'flex', flexDirection: 'column', padding: '20px 30px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '25px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
           ...
           <div>
              <h1 style={{ fontSize: '24px', fontWeight: '900', letterSpacing: '-0.02em' }}>Console <span style={{ color: 'var(--accent)' }}>Système & Audit</span></h1>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', opacity: 0.3 }}>
                 <Activity size={10} />
                 <p className="caption" style={{ textTransform: 'uppercase', letterSpacing: '1px' }}>Opsec PRO Engine — Kernel actif</p>
              </div>
           </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '15px', padding: '15px', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid var(--border)' }}>
        <div style={{ position: 'relative', flex: 1 }}>
          ...
          <input 
            type="text" 
            placeholder="Filtrer les logs système ou audit..." 
        ...
        <div style={{ display: 'flex', background: 'var(--bg-input)', borderRadius: '8px', padding: '2px' }}>
          {(['all', 'success', 'error', 'info', 'security'] as const).map(t => (
            <button 
              key={t}
              ...
              style={{
                ...
                color: typeFilter === t ? (t === 'security' ? '#ffd700' : 'var(--accent)') : 'var(--text-dim)',
                borderRadius: '8px',
