...
export const Settings = () => {
  ...
  const t = {
    fr: {
      ...
      privateMode: "Mode Furtif (Stealth)",
      privateDesc: "Masque 'Opsec Pro' de vos bios, statuts et activités Discord.",
      appDetection: "Détection d'Apps & Jeux",
      appDetectionDesc: "Permet au bot de détecter vos jeux actifs pour votre statut.",
      ...
      resetSettingsDesc: "Rétablit les réglages visuels et sonores par défaut (sans toucher aux comptes ni à la licence).",
      language: "Langue",
    ...
    en: {
      ...
      privateMode: "Stealth Mode",
      privateDesc: "Suppresses 'Opsec Pro' from your Discord bios, status and activities.",
      appDetection: "App & Game Detection",
      appDetectionDesc: "Detect active games and processes for your status.",
      sentinelMode: "Sentinel Duo (Anti-Kick)",
      sentinelDesc: "Allows DM group anti-kick protection in Raid Hub → Network Pro.",
      resetSettings: "Reset Preferences",
      resetSettingsDesc: "Restores visual and audio defaults (preserves accounts and license key).",
      language: "Language",
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: '25px' }}>
              <div className="glass-card settings-card-glow" style={{ padding: '30px', borderRadius: '18px' }}>
                 ...
                 <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    ...
                    <div className="settings-item-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderRadius: '12px' }}>
                       <div style={{ maxWidth: '80%' }}>
                          <p style={{ fontSize: '13px', fontWeight: '800', color: 'white' }}>{t.privateMode}</p>
                          <p className="caption" style={{ opacity: 0.5, textTransform: 'none', marginTop: '4px', lineHeight: '1.4' }}>{t.privateDesc}</p>
                       </div>
                       <div onClick={() => updateSetting('privateMode', !settings.privateMode)} className={`nighty-toggle ${settings.privateMode ? 'active' : ''}`}>
                          <div className="nighty-toggle-handle"></div>
                       </div>
                    </div>
                    <div className="settings-item-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderRadius: '12px' }}>
                       <div style={{ maxWidth: '80%' }}>
                          <p style={{ fontSize: '13px', fontWeight: '800', color: 'white' }}>{t.appDetection}</p>
                          <p className="caption" style={{ opacity: 0.5, textTransform: 'none', marginTop: '4px', lineHeight: '1.4' }}>{t.appDetectionDesc}</p>
                       </div>
              ...
              {/* Dedicated Onboarding Tour & Tutorial Section */}
              <div className="glass-card settings-card-glow" style={{ padding: '30px', borderRadius: '18px', gridColumn: '1 / -1' }}>
                 <h3 style={{ marginBottom: '25px', display: 'flex', alignItems: 'center', gap: '12px', fontSize: '15px', fontWeight: '900', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '12px' }}>
                    <Sparkles size={20} color="#00ffcc" /> {settings.language === 'fr' ? "GUIDE D'INITIATION & TUTORIEL" : "ONBOARDING & INTERACTIVE TOUR"}
                 </h3>
                 <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '15px' }}>
                    <div className="settings-item-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderRadius: '12px' }}>
                       <div style={{ maxWidth: '65%' }}>
                          <p style={{ fontSize: '13px', fontWeight: '800', color: 'white' }}>
                             {settings.language === 'fr' ? "Guide Interactif de Démarrage" : "Interactive Onboarding Tour"}
                          </p>
                          <p className="caption" style={{ opacity: 0.5, textTransform: 'none', marginTop: '4px', lineHeight: '1.4' }}>
                             {settings.language === 'fr' 
                                ? "Revoir le tutoriel pas à pas (Command Palette Ctrl+K, In-Chat, Moteurs, Proxies 1:1)." 
                                : "Replay step-by-step tour (Command Palette Ctrl+K, In-Chat, Engines, 1:1 Proxies)."}
                          </p>
                       </div>
                       <button
                          onClick={() => {
                             window.dispatchEvent(new CustomEvent('opsec:open-onboarding'));
                             showToast(settings.language === 'fr' ? "Guide d'initiation lancé !" : "Onboarding tour started!", 'success');
                          }}
                       ...
                       >
                          <Sparkles size={14} />
                          {settings.language === 'fr' ? "Lancer le Guide" : "Start Tour"}
                       </button>
                    </div>
                    ...
                    <div className="settings-item-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderRadius: '12px' }}>
                       <div style={{ maxWidth: '65%' }}>
                          <p style={{ fontSize: '13px', fontWeight: '800', color: 'white' }}>
                             {settings.language === 'fr' ? "Réafficher au prochain démarrage" : "Show on next startup"}
                          </p>
                          <p className="caption" style={{ opacity: 0.5, textTransform: 'none', marginTop: '4px', lineHeight: '1.4' }}>
                             {settings.language === 'fr' 
                                ? "Réinitialise l'état pour réafficher le guide automatiquement à la prochaine connexion." 
                                : "Resets the flag so the tour automatically launches on next login."}
                          </p>
                       </div>
                       <button
                          onClick={() => {
                             localStorage.removeItem('opsec_onboarding_completed');
                             showToast(settings.language === 'fr' ? "Le guide réapparaîtra au prochain démarrage" : "Tour will display on next login", 'success');
                          }}
                       ...
                       >
                          {settings.language === 'fr' ? "Réinitialiser" : "Reset Flag"}
                       </button>
                    </div>
                 </div>
              </div>
         ...
         {activeSubTab === 'Visual' && (
            <div className="glass-card settings-card-glow animate-slide-right" style={{ padding: '30px', borderRadius: '18px' }}>
               ...
               {/* Safe Factory Reset of Preferences */}
               <div className="glass-card settings-card-glow" style={{ padding: '24px', borderRadius: '18px', marginTop: '20px', border: '1px solid rgba(239, 68, 68, 0.25)', background: 'rgba(239, 68, 68, 0.02)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                     <div>
                        <h3 style={{ margin: '0 0 6px 0', fontSize: '15px', fontWeight: '900', color: '#ef4444', display: 'flex', alignItems: 'center', gap: '10px' }}>
                           <RefreshCw size={18} /> {t.resetSettings}
                        </h3>
                        <p className="caption" style={{ margin: 0, opacity: 0.6, textTransform: 'none' }}>
                           {t.resetSettingsDesc}
                        </p>
                     </div>
                     <button
                        onClick={async () => {
                           try {
                              const res = await (window as any).electronAPI.resetSettings();
                              if (res.success && res.data) {
                                 updateSetting('themeBackground', res.data.themeBackground || '');
                                 updateSetting('themeBlur', res.data.themeBlur || 10);
                                 updateSetting('themeOpacity', res.data.themeOpacity || 0.8);
                                 updateSetting('audioVolume', res.data.audioVolume || 0.5);
                                 updateSetting('audioEnabled', res.data.audioEnabled ?? true);
                                 updateSetting('silentMode', res.data.silentMode ?? true);
                                 updateSetting('privateMode', res.data.privateMode ?? false);
                                 updateSetting('cyberCursorEnabled', false);
                                 updateSetting('customCursorUrl', '');
                                 showToast(settings.language === 'fr' ? 'Préférences réinitialisées aux valeurs par défaut !' : 'Preferences reset to defaults!', 'success');
                              }
                           } catch (err) {
                     ...
                     >
                        {settings.language === 'fr' ? "RÉINITIALISER LES PRÉFÉRENCES" : "RESET PREFERENCES"}
                     </button>
                  </div>
               </div>
            </div>
