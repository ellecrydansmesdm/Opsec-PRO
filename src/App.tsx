...
import { UpdateModal, UpdateData } from '@/components/ui/UpdateModal';
...
function App() {
  ...
  // Auto-launch sleek onboarding tour on first login
  useEffect(() => {
    if (isAuthenticated && user) {
      try {
        const completed = localStorage.getItem('opsec_onboarding_completed');
        if (!completed) {
  ...
  // Global event listener to trigger onboarding from Settings / CommandPalette
  useEffect(() => {
    const handleOpen = () => setShowOnboarding(true);
    window.addEventListener('opsec:open-onboarding', handleOpen);
    return () => window.removeEventListener('opsec:open-onboarding', handleOpen);
  }, []);

  // Automatic background update check on app launch & periodic check (every 15 min)
  useEffect(() => {
        ...
        window.electronAPI.checkForUpdates()
          .then((data) => {
            if (data && data.updateAvailable) {
              const dismissed = localStorage.getItem('opsec_dismissed_update');
    ...
    <div style={{ 
      ...
      {requiresLicense ? (
        ...
        <div className="app-container" style={{ flex: 1, display: 'flex', overflow: 'hidden', paddingTop: 0 }}>
          ...
          <main className="main-content" style={{ flex: 1, overflowY: 'auto', position: 'relative' }}>
            ...
            <KeepAlivePage active={activeTab === 'Tools'}>
              {user && <ToolsHub showToast={showToast} onConfirm={(data) => setConfirmData(data)} />}
            </KeepAlivePage>
      ...
      <CommandPalette 
        onNavigate={(tab) => {
          ...
          else if (tab === 'inchat' || tab === 'macros' || tab === 'voice' || tab === 'engine') setActiveTab('Engine');
          else if (tab === 'raid' || tab === 'cloner' || tab === 'lastword' || tab === 'spam' || tab === 'purge') setActiveTab('Raid');
          else if (tab === 'tools' || tab === 'pomelo' || tab === 'sanitizer' || tab === 'badges' || tab === 'spotify') setActiveTab('Tools');
